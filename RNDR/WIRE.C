...
{
    i32 rx = fx_mul_q16(x, cosy) - fx_mul_q16(z, siny);
    i32 rz = fx_mul_q16(x, siny) + fx_mul_q16(z, cosy);
    i32 ry = y;
    *ox = rx;
    *oy = fx_mul_q16(ry, cosx) - fx_mul_q16(rz, sinx);
    *oz = fx_mul_q16(ry, sinx) + fx_mul_q16(rz, cosx);
}
...
static long tri_area2(int x0, int y0, int x1, int y1, int x2, int y2)
{
